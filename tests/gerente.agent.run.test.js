...
describe('runAgentTurn', () => {
  ...
  it('ferramenta de escrita vira ação pendente e não chama a RPC', async () => {
    ...
    expect(result.pendingAction).toEqual({ id: 'act-1', summary: 'Pausar "Refri 2L" no cardápio digital', expires_at: '2026-09-02T15:10:00Z', effect: 'Para de aparecer para o cliente em todo o cardápio digital. Continua no PDV para venda no balcão.' });
  ...
  it('não devolve quickReplies quando criou ação pendente', async () => {
    ...
    expect(result.quickReplies).toEqual([]);
    expect(result.pendingAction.effect).toBe('Para de aparecer para o cliente em todo o cardápio digital. Continua no PDV para venda no balcão.');
  });
