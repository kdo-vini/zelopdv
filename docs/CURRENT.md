...
- Progresso da continuação (roteiro em `docs/HANDOFF-design-system.md`; cada
  ...
  - **Mockup 05 — Perfil, Assinatura e Ferramentas** aprovado pelo dono. Ferramentas entregue atrás da flag: hub com cards e selos, Precificação com tabela/cartões e números em Mono, Cardápio com editor responsivo e switches com mola. Perfil também entregue: cinco abas em `UnderlineTabs`, troca com `blurSwap`, cartões por assunto, `MorphButton` no salvamento, switches com mola, zona de perigo recolhida e versão de subusuário preservada; no celular, “Salvar alterações” fica depois dos cartões. Assinatura/Pix entregue e revisada conforme as seções 2, 6, 13 e 14 do mockup: ramo próprio `$zeloSurface`, etapas em `UnderlineTabs` ligadas aos handlers existentes, pacotes em linha e resumo lateral com `MoneyText` no desktop, status/bolinhas compactos e pacotes empilhados com resumo fixo no celular, seleção navy com borda neutra nos demais pacotes, `EntitlementLossWarning`, revisão com vencimento, CPF/CNPJ condicional e meios Pix/Cartão. `/assinatura/sucesso` fecha o fluxo com cartão responsivo, check desenhado, contador Mono e progresso sincronizado; tracking, deduplicação e redirecionamentos não mudaram. Só apresentação; textos e preços de `pricing.js`, leituras, escritas, endpoints e handlers de cobrança permanecem iguais. O bloco do onboarding não mudou. Perfil legado comparado ao commit-base pelo harness + `compare-screens.mjs`: zero diferenças em desktop e celular. A revisão de Assinatura foi comparada ao `cd1325c`: zero diferenças em desktop, celular e no estado posterior ao clique em “Copiar código Pix”. Confirmação legada mantém o mesmo layout em desktop e celular; o comparador aponta apenas contador/barra capturados em instantes diferentes, animação conhecida do harness. Validação do mockup 05: `check` (0 erros; 1 warning antigo em Relatórios), `check:ui` (33 arquivos limpos), Vitest (295 arquivos / 2313 testes passando; 3 arquivos / 3 testes ignorados) e build completos; no Windows, o adaptador Vercel exigiu um shim local não versionado para substituir symlinks por junctions. `/gestao/caixa` é o próximo item da Fase 4.
  - **Sobras da Fase 4**: `/gestao/caixa` entregue atrás da flag com totais hierarquizados, valores em Mono, reconciliação destacada, ação de fechamento em `MorphButton` e layout móvel que respeita a bottom nav. `/gestao/acessos` também entregue: `UnderlineTabs` com contadores, cartões claros de cargos/permissões, estados ativos com contraste semântico, tabela de usuários no desktop e cartões empilhados no celular, mantendo convites, autosave, confirmações, endpoints e RBAC intactos. As duas telas nascem com o movimento do sistema e preservam a bottom nav original. Legado de Caixa e Acessos comparado pelo harness + `compare-screens.mjs`: zero diferenças nos estados cobertos em desktop e celular. `/gestao/empresas` é o próximo item.
  - **Onboarding — cadastro → primeira venda** (mockup 06 aprovado em 2026-09-26, `docs/design-system/mockups/06-onboarding.html`), tela a tela atrás da flag, só apresentação (eventos de onboarding na mesma ordem):
    - ✅ **Cadastro e volta do Google**: "Criar conta" em `MorphButton` (carregando → check "Conta criada"), faixa de sucesso com ícone; `/auth/callback` com a marca e o anel girando no lugar do spinner cru. Não existe tela de "confirme seu e-mail" (o cadastro já devolve a sessão).
    - ✅ **Wizard de 2 passos + boas-vindas**: folha inferior no celular (flutua acima do teclado via `visualViewport` e acima da bottom nav), cartão de 440 px no desktop; barra de 2 segmentos com mola no lugar das bolinhas; pergunta em Mono; "Continuar"/"Começar a usar" em `MorphButton` (carregando enquanto salva); conteúdo troca com blur; chegada desenha o check; "Como conheceu" em chips com troca para "Valeu por contar!". Harness: `WIZARD=1`, `SLOW=<ms>`, passo `type:` e `ROUTE` com query.
    - ✅ **Frente de Caixa no primeiro uso**: estado vazio no sistema (botão navy "Cadastrar primeiro produto", "Ou venda avulsa" discreto, prévia tracejada), link "+ Nova categoria" mais quieto no cadastro rápido, dica do primeiro produto como bloco navy colado no tile com o filete dos 8 s, "Antes de receber" + `MorphButton` no Abrir caixa quando ele vem da barreira do pagamento, e o pagamento entra com blur logo depois (`continuing`), lendo como a mesma folha. **Achado do implementador, não corrigido no legado:** a dica pode não aparecer porque `{@const isCoachmark = isCoachmarkProduct(produto)}` não acompanha `coachmarkProductId`; no ramo Zelo a condição agora é lida direto. Vale conferir em produção. Harness: eco do insert de produto e passo `fill:`.
    - ✅ **Terminar de configurar**: cartão com contador `feitos/total` em Mono, barra que enche com mola, linhas de 50 px com check verde/círculo e chevron, concluídos riscados. Mesmos itens, links e regra de visibilidade (só titular, some quando tudo feito). **Para o dono decidir:** no celular o Dashboard empilha as colunas e o checklist fica abaixo de KPIs, gráfico e atividade, fora da primeira tela. Harness: `CHECKLIST=1` (com os dados do mock a contagem dá 2/4, porque bobina vazia já conta como feita — regra de `onboardingChecklist.js`).
    - Onboarding completo atrás da flag. Legado de cada tela comparado antes/depois pelo harness + `compare-screens.mjs` (0 diferenças fora ruído de captura).
