...
- Validação final local do Zelinho: 18 testes direcionados passaram; `npm run check` permanece com 0 errors / 111 warnings. `npm test` teve 295/298 testes verdes; os três timeouts em `guards.ensureActiveSubscription`, `guards.offline` e `guards.zelomenu` são concorrentes/fora da superfície do gerente e exigem estabilização antes de considerar a branch totalmente verde.
