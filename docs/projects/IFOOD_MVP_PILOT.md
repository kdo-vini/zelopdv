...
**Decisão atual:** **GO parcial (schema + worker live+ready)** — migrations aplicadas e worker Dokploy com `/health/live` 200 e `/health/ready` 200 enquanto o probe for fresco (TTL default 600s > intervalo 300s); **não é GO completo**. `IFOOD_WORKER_PROCESS_COMMANDS=1` no Dokploy (2026-09-17); inbox e adapter HTTP default off; shadow/piloto/soak pendentes.
...
| 3 | Shadow mode (comandos/presença off) | Parcial | Ops 2026-09-17 | Código do worker aceita flags `IFOOD_WORKER_PROCESS_INBOX` / `PROCESS_COMMANDS` / `ENABLE_HTTP_ADAPTER` (default off). Dokploy ligou **só** `IFOOD_WORKER_PROCESS_COMMANDS=1`. Inbox e adapter HTTP não assumidos. Sem o par `IFOOD_CLIENT_*` o adapter fica null e o hook default de commands **não** envia HTTP. Presença continua off. Detalhe: `docs/operations/IFOOD.md` § Flags de ciclo. |
...
existem no código e ficam **off** por default. Dokploy ligou
`IFOOD_WORKER_PROCESS_COMMANDS=1` (2026-09-17); inbox/adapter HTTP continuam
off até autorização. Sem GO completo.
...
PENDENTE PARA GO COMPLETO:
  1. IFOOD_WORKER_PROCESS_INBOX + ENABLE_HTTP_ADAPTER + IFOOD_CLIENT_*
     (PROCESS_COMMANDS já =1 no Dokploy; sozinha não envia HTTP)
  2. merchant/sandbox atribuído
  3. Shadow → loja piloto → soak → sign-off GO pleno
  4. Só então liberar self-service gradual

BLOQUEADO AGORA: PROCESS_INBOX e ENABLE_HTTP_ADAPTER não assumidos;
IFOOD_CLIENT_ID/SECRET não colados aqui; sem merchant sandbox;
shadow/piloto/soak não fechados. Ready=200 não é ciclo operacional.
```
