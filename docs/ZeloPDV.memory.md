...
- Pedido manual (2026-09-05): usa `order.create` na outbox,
  ...
  manual. Online, registra apenas o aparelho e sincroniza sem habilitar o modo
  offline; sem conexão, requer preparação em Perfil > Integrações. As migrations
  `20260905210000` e `20260905214500` já foram aplicadas; a segunda revalida a
  assinatura ativa no replay no projeto compartilhado. Consultar CURRENT para
  rollout do cliente. Consulta offline da fila não implica suporte offline
...
- A impressão remota do ZeloPDV usa `zelo_print_jobs` e uma estação opt-in no navegador do computador. A aba autenticada e o Zelo Impressão precisam permanecer abertos. Pedidos canônicos de Menu, Chat/WhatsApp e Mesas após envio à cozinha são observados globalmente, não por `/app/pedidos`, e usam `zelo_orders.id` como chave da fila. Recibos e comandas solicitados em outros aparelhos expiram em duas horas. `PRINT_OUTCOME_UNKNOWN` nunca repete; falhas seguras tentam até três vezes. Somente o titular pode registrar/operar uma estação; subusuários podem enfileirar dentro do owner resolvido no servidor.
