...
| 4 | Superfície App atrás da flag (estrutura → `/app` → modais → operação → gestão → relatórios → conta) | **Em andamento** — sidebar navy (métricas do mockup) e `/app` (desktop + mobile) no layout do mockup, com o sistema de movimento; modais do PDV (pagamento, sucesso, quantidade, avulso, caixa, montável, novo produto), toasts, confirmação, offline e bottom nav no sistema; mesas, pedidos, cozinha e gestão (dashboard, produtos, pessoas, estoque, despesas, fichário, cadastro de mesas, Fechar Caixa e Controle de Acessos), relatórios, Ferramentas (hub, Precificação e Cardápio), Perfil, Assinatura/Pix e confirmação de pagamento reescritos; Assinatura revisada conforme o mockup 05 com `UnderlineTabs`, pacotes em linha + resumo lateral no desktop e status/etapas compactos + resumo fixo no celular; onboarding (mockup 06): cadastro, volta do Google, wizard de 2 passos com boas-vindas e primeiro uso da Frente de Caixa (vazio, dica, abrir caixa antes de receber) e "Terminar de configurar"; demais telas internas legíveis pela camada de compatibilidade (`compat-app.css`), reescrita tela a tela pendente |
