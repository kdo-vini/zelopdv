...
| 4 | Superfície App atrás da flag (estrutura → `/app` → modais → operação → gestão → relatórios → conta) | **Em andamento** — sidebar navy (métricas do mockup) e `/app` (desktop + mobile) no layout do mockup, com o sistema de movimento; modais do PDV (pagamento, sucesso, quantidade, avulso, caixa, montável, novo produto), toasts, confirmação, offline e bottom nav no sistema; mesas, pedidos, cozinha e dashboard da gestão reescritos (mockups 01–03 aprovados); demais telas internas legíveis pela camada de compatibilidade (`compat-app.css`), reescrita tela a tela pendente |
