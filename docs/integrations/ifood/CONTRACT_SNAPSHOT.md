...
| --- | --- | --- | --- |
| `GET` | `/merchants` | `200`, uma loja | Array plano de merchants (`id`, `name`, `corporateName`), sem header `Link`. Em 2026-09-16, `?page=2&size=1` retornou `200` com array vazio (a conta tem 1 loja), indício de que `page`/`size` são aceitos; semântica completa não confirmada, por isso a verificação de conexão usa `/status`. **Passou de código morto a uso real em 2026-09-17**: `connectionService.js` (`discoverMerchants`) chama esta rota para listar lojas já autorizadas pelo lojista no lado do iFood, filtra as que nenhuma empresa do Zelo reivindicou ainda (via a nova RPC `list_ifood_claimed_merchant_ids_v1`) e oferece essa lista como picker de um clique no wizard `not_connected` — cortando a digitação do `merchantId` quando a autorização já existe. Segue limitado a 25 itens por página por causa da paginação não confirmada citada acima. **Isolamento entre tenants (2026-09-18, achado em revisão antes do merge):** o app centralizado retorna lojas autorizadas de **todos** os clientes do Zelo, não só do tenant que está perguntando — filtrar apenas por "não reivindicada ainda" não bastava e vazava nome/ID de lojas de outras empresas para qualquer conta autenticada, com risco de sequestro (reivindicar a loja de outro dono com um clique). Corrigido exigindo correspondência entre `name`/`corporateName` do merchant e `empresa_perfil.nome_exibicao`/`razao_social` do tenant (normalizado: minúsculo, sem acento, sem sufixo societário) antes de qualquer merchant entrar na lista; sem nome próprio cadastrado, a lista vem vazia e degrada pro campo manual. |
