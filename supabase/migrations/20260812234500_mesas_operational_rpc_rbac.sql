-- Align the existing Mesa stock RPCs with the owner/capability contract from
-- 20260812233000_mesas_operational_rbac.sql.
--
-- Before this migration the functions compared auth.uid() directly with the
-- owner columns, so a legitimate sub-user could open a comanda but could not
-- add items, reserve stock, or cancel/close it. The browser consumers are
-- unchanged. These SECURITY DEFINER functions are intentionally narrow,
-- owner-resolved, fixed-search-path, and capability-checked before any write.
-- service_role keeps its existing maintenance bypass.
