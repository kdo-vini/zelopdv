  ...
  v_fulfillment := coalesce(p_order->'fulfillment', '{}'::jsonb)
    || jsonb_build_object(
      'ifood', jsonb_build_object(
        'displayId', p_order->>'displayId',
        'externalOrderId', p_external_order_id,
        'preparationStartAt', p_order->>'preparationStartAt',
        'scheduled', coalesce((p_order->>'scheduled')::boolean, false),
        'scheduleStart', coalesce(
          p_order#>>'{fulfillment,schedule,start}',
          p_order#>>'{fulfillment,schedule,deliveryDateTimeStart}'
        ),
        'scheduleEnd', coalesce(
          p_order#>>'{fulfillment,schedule,end}',
          p_order#>>'{fulfillment,schedule,deliveryDateTimeEnd}'
        ),
        'pickupCode', p_order->>'pickupCode',
        'deliveryCode', p_order->>'deliveryCode'
      )
    );
