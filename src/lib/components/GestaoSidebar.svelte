...
<style>
  ...
  .zelo-word-store { font: var(--type-caption); letter-spacing: var(--type-caption-tracking); color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  /* Navigation metrics measured on the approved mockup (zelopdv-app-light.html: .sb, .sb h6, .sb a):
     244px wide · items 13.5px/500 (type-label), 38px tall, 18px icons, gap 11px · section labels in the
     eyebrow role (Geist Mono 10.5/500, .12em, uppercase — Mono is the brand voice, docs/DESIGN_SYSTEM.md → Tipografia).
  ...
  .zelo-sidebar .sidebar-nav :global(p) { margin: 16px 0 6px; padding: 0 10px; font: var(--type-eyebrow); letter-spacing: var(--type-eyebrow-tracking); text-transform: uppercase; }
  .zelo-sidebar .sidebar-nav :global(ul > li) { margin-top: 0; }
  .zelo-sidebar .sidebar-nav :global(li > a), .zelo-sidebar .sidebar-nav :global(li > button) { height: 38px; gap: 11px; padding: 0 10px; border-radius: 10px; font: var(--type-label); line-height: 1; letter-spacing: var(--type-label-tracking); }
