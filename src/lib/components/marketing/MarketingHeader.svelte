<script>
  ...
  $: navLinks = [
    ...
    { href: '/extensoes', label: 'Extensões' },
...
>
  <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-6">
    <a href="/" class="flex items-center shrink-0" aria-label="Zelo PDV — início">
      <img src="/logo-horizontal.png" alt="Zelo PDV" class="h-10 md:h-11 w-auto" />
    </a>
    ...
    <div class="flex items-center gap-3 md:gap-4 shrink-0">
      <a
        href="/login"
        class="hidden md:block text-sm font-medium transition-colors hover:text-sky-300"
        style={`color: ${isLight ? 'var(--blog-text)' : 'rgba(255,255,255,0.92)'}`}
      >
        Entrar
      </a>
      <a
        href="/cadastro"
        class="px-4 md:px-5 py-2 md:py-2.5 text-sm font-semibold text-white bg-sky-500 hover:bg-sky-400 rounded-full shadow-lg shadow-sky-950/40 transition-all"
      >
        Testar grátis
      </a>
      <button
