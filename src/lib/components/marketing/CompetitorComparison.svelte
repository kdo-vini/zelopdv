...
>
  ...
  <main>
    <!-- Hero -->
    <section class="relative pt-32 pb-16 overflow-hidden border-b border-mk-ink/5">
      <div class="max-w-3xl mx-auto px-6">
        <p class="text-xs tracking-wider" style="color: var(--text-muted);">
          Comparativo · Preço checado em {comparison.priceCheckedAt} ·
          <a href="#fontes" class="text-mk-highlight hover:text-mk-highlight-soft underline underline-offset-4">Fontes</a>
        </p>
        ...
        <h1 class="mt-4 text-4xl md:text-6xl font-bold text-mk-ink tracking-tight leading-tight" style="text-wrap: balance;">
        ...
        <div class="mt-10 flex flex-col sm:flex-row gap-4 items-start">
          <a href="#comparativo" class="px-6 py-3 rounded-full font-semibold border border-mk-accent/40 bg-mk-accent/10 text-mk-highlight hover:bg-mk-accent/15 transition-colors">
            Ver comparativo →
          </a>
          <a href={cadastroHref} class="px-6 py-3 text-mk-highlight hover:text-mk-highlight-soft font-semibold underline underline-offset-4 transition-colors" on:click={() => trackSignupCta('vs_hero')}>
    ...
    <section class="py-24 border-b border-mk-ink/5">
      <div class="max-w-4xl mx-auto px-6">
        <h2 class="text-3xl md:text-4xl font-bold text-mk-ink mb-8">{comparison.introTitle}</h2>
    ...
    <!-- Comparison table -->
    <section id="comparativo" class="py-24 border-b border-mk-ink/5" style="background: var(--bg-panel);">
      <div class="max-w-5xl mx-auto px-6">
        <div class="max-w-3xl mb-12">
          <h2 class="text-3xl md:text-4xl font-bold text-mk-ink mb-6">Zelo PDV × {comparison.competitor}, lado a lado</h2>
        ...
        <div class="overflow-hidden rounded-3xl border" style="background: var(--bg-card); border-color: var(--border-card);">
          <div class="grid grid-cols-[1.2fr_1fr_1fr] text-sm font-semibold" style="background: var(--bg-panel); color: var(--text-muted);">
            ...
            <div class="p-4 md:p-5 text-mk-highlight">Zelo PDV</div>
          </div>
            ...
            <div class="grid grid-cols-[1.2fr_1fr_1fr] border-t" style="border-color: var(--border-card);">
              ...
              <div
                class={cn(
                  ...
                  row.advantage === 'zelo' ? 'font-semibold text-mk-ink bg-mk-accent/5' : 'text-foreground'
                )}
              >
                {row.zelo}
              </div>
    ...
    <section class="py-24 border-b border-mk-ink/5">
      <div class="max-w-7xl mx-auto px-6">
        <div class="max-w-3xl mb-14">
          <h2 class="text-3xl md:text-4xl font-bold text-mk-ink mb-6">{comparison.reasonsTitle}</h2>
        </div>
          ...
          {#each comparison.reasons as reason}
            <article class="rounded-3xl border p-7" style="background: var(--bg-card); border-color: var(--border-card);">
              <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-5 bg-mk-accent/10 border border-mk-accent/20">
                <svelte:component this={resolveAppIcon(reason.icon)} class="size-7 text-mk-highlight" aria-hidden="true" />
              </div>
              <h3 class="text-2xl font-semibold text-mk-ink mb-3">{reason.title}</h3>
        ...
        {#if comparison.fairnessNote}
          <div class="mt-10 rounded-3xl border p-6 md:p-7" style="background: var(--bg-panel); border-color: var(--border-subtle);">
            <p class="text-sm uppercase tracking-[0.2em] mb-3 text-mk-highlight">Sendo justo</p>
    ...
    <section class="py-20 border-b border-mk-ink/5" style="background: var(--bg-panel);">
      <div class="max-w-4xl mx-auto px-6">
        <div class="rounded-2xl border p-6 md:p-7" style="background: var(--bg-card); border-color: var(--border-card);">
          <h2 class="text-2xl md:text-3xl font-bold text-mk-ink mb-3">R$ 59/mês — plano único</h2>
          ...
          <a href="/precificacao" class="text-mk-highlight hover:text-mk-highlight-soft font-semibold underline underline-offset-4">
    ...
    <section id="faq" class="py-24 border-b border-mk-ink/5">
      <div class="max-w-4xl mx-auto px-6">
        <div class="text-center mb-12">
          <h2 class="text-3xl md:text-4xl font-bold text-mk-ink mb-4">As perguntas mais comuns: Zelo PDV × {comparison.competitor}</h2>
        </div>
    ...
    <section class="py-24 border-b border-mk-ink/5">
      <div class="max-w-4xl mx-auto px-6 text-center">
        <div class="border-t pt-12" style="border-color: var(--border-subtle);">
          <h2 class="text-3xl md:text-5xl font-bold text-mk-ink mb-6">{comparison.finalCtaTitle}</h2>
      ...
      <section class="py-20 border-b border-mk-ink/5" style="background: var(--bg-panel);">
        <div class="max-w-7xl mx-auto px-6">
          <div class="max-w-3xl mb-10">
            <h2 class="text-3xl md:text-4xl font-bold text-mk-ink mb-4">Compare o Zelo PDV com outros sistemas</h2>
            <p class="text-lg leading-relaxed" style="color: var(--text-muted);">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {#each otherComparisons as other}
              ...
              >
                <p class="text-lg font-semibold text-mk-ink mb-1 group-hover:text-mk-highlight transition-colors">
          ...
          <div class="mt-8">
            <a href="/comparativos" class="text-mk-highlight hover:text-mk-highlight-soft font-semibold underline underline-offset-4">
          ...
          <ul class="space-y-1">
            {#each comparison.sources as source}
              <li class="text-xs" style="color: var(--text-muted);">
                <a href={source.url} target="_blank" rel="nofollow noopener" class="hover:text-mk-highlight underline underline-offset-2">{source.label}</a>
              </li>
            {/each}
          </ul>
