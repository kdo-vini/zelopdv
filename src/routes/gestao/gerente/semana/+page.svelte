<script>
  ...
  import { buildWeekReport, getWeekStart, shiftWeek } from '$lib/gerente/weekReport.js';
  ...
  $: selectedWeek = requestedWeek && /^\d{4}-\d{2}-\d{2}$/.test(requestedWeek) ? requestedWeek : currentWeek;
...
<section class="week-page">
  ...
  <div class="mb-6 flex items-end justify-between border-b border-slate-700/60 pb-4">
    <div><p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-1">Gestão / Zelinho</p><h1 class="text-xl font-bold text-slate-100 tracking-tight">Resumo semanal</h1></div>
    {#if !loading && !error}<div class="share-actions"><Button variant="outline" size="sm" on:click={copySummary}><Copy />Copiar resumo</Button><Button variant="outline" size="sm" on:click={shareSummary}><Share2 />WhatsApp</Button></div>{/if}
  </div>
...
<style>
  .week-page { max-width: 980px; margin: 0 auto; }.share-actions { display: flex; gap: 6px; }.partial { margin: 14px 0; padding: 8px 10px; border-left: 2px solid var(--status-warning-text); background: var(--status-warning-bg); color: var(--text-label); font-size: 13px; }.opening, .panel, .next { margin-top: 16px; padding: 16px; border: 1px solid var(--border-card); border-radius: 8px; background: var(--bg-card); }.opening { border-top: 2px solid var(--primary); color: var(--text-label); }.stat-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; margin-top: 16px; }.stat-grid > div :global(.delta) { display: block; margin: 5px 2px 0; }.split { display: grid; grid-template-columns: 1.15fr .85fr; gap: 16px; }.panel h2, .next h2 { margin: 0 0 12px; color: var(--text-main); font-size: 14px; }.products, .signals { display: grid; gap: 10px; margin: 0; padding: 0; list-style: none; }.products li { display: grid; grid-template-columns: minmax(0, 1fr) 42%; gap: 12px; align-items: center; }.products strong, .products span { display: block; }.products strong { color: var(--text-label); font-size: 13px; }.products span, .muted { color: var(--text-muted); font-size: 12px; }.product-rank { display: flex; align-items: center; gap: 6px; }.product-rank i { display: block; height: 5px; min-width: 8px; background: var(--primary); border-radius: 3px; }.product-rank b { color: var(--text-muted); font-size: 12px; white-space: nowrap; }.product-rank b.up { color: var(--status-success-text); }.product-rank b.down { color: var(--status-warning-text); }.signals a { display: flex; align-items: center; gap: 8px; color: var(--text-label); font-size: 13px; text-decoration: none; }.signals a:hover { color: var(--primary); }.tag { padding: 2px 7px; border-radius: 99px; font-size: 9px; font-weight: 700; text-transform: uppercase; }.tag.critical { color: var(--status-error-text); background: var(--status-error-bg); }.tag.attention { color: var(--status-warning-text); background: var(--status-warning-bg); }.tag.info { color: var(--primary); background: var(--accent-light); }.next p { color: var(--text-label); font-size: 13px; }.empty, .skeleton { min-height: 160px; display: grid; place-items: center; margin-top: 16px; border: 1px dashed var(--border-strong); border-radius: 8px; color: var(--text-muted); }.skeleton { animation: pulse 1.2s ease-in-out infinite; background: var(--bg-panel); } @keyframes pulse { 50% { opacity: .5; } } @media (max-width: 700px) { .stat-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }.split { grid-template-columns: 1fr; }.share-actions { flex-wrap: wrap; justify-content: flex-end; } } @media (max-width: 420px) { .share-actions :global(button) { padding-inline: 7px; }.week-page { max-width: 100%; } }
</style>
